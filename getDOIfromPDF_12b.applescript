...
tell application "BibDesk"
	...
	tell theDoc
		...
		if theAdsurl is "" then
			...
			--set TheCommand to "curl -Ls -o /dev/null -w %{url_effective} " & the quoted form of theBibtexUrl
			...
			if theNewBibtexString2 = "" then
				...
				if theExistingAbstract is not "" then
					...
					-- the redirect-following in getADSAbstract() is probably happening from the second time here, haven't figured out yet how to remove the redundancy
					--display dialog theMessage buttons {"¥"} default button 1 giving up after 4
...
on getADSAbstract(theAdsurl)
	...
	--display dialog "getADSAbstract: " & theAdsurl buttons {"¥"} default button 1 giving up after 5
...
on getBibtexEntry(theBibtexUrl)
	...
	do shell script TheCommand2
	set theBibtexString2 to the result
	--display dialog "getBibtexEntry (2): " & theBibtexString2 buttons {"¥"} default button 1 giving up after 5
	
	--	tell application "TextEdit"
	--		text of theBibtexString2
	--	end tell
